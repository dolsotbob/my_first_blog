...
export const uploadFileToIPFS = async () => {
  const fileStream = await createReadStream();   // import한 createReadStram()을 호출해 이미지 파일을 스트림 형식으로 읽는다
  ...
  formData.append('name', 'MyNFT_Image');  // name이라는 key에 'MynFT_Image"라는 값을 추가함; 이는 Pinata에서 파일 이름으로 사용됨(필수는 아님)
