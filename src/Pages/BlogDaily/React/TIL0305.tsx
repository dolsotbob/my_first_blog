...
const TIL0305 = () => {
  return (
    <div className='BlogDaily'>TIL0305
      ...
      <h4>2 티어 아키텍처</h4>
      <p>2-Tier 아키텍처</p>
      <ul><li>빈번한 업데이트가 필요한 경우, 리소스가 존재하는 곳과 리소스를 사용하는 앱을 분리시키는 것이 유리하다</li>
        <li>리소스가 존재하는 곳(서버)과 리소스를 사용하는 앱을 분리시킨 것을 2-Tier 아키넥처 혹은 클라이언트 - 서버 아키텍처라고 함</li>
        <li>커피숍 손님(Client)은 리소스를 가지고 있는 점원(Server)에게 물품을 "요청"해야 함. 요청에 따라 점원(Server)은 리소스를 담아 응답함</li></ul>

      <p>3-Tier 아키텍처</p>
      <ul><li>보통 서버는 리소스를 전달해줄 뿐 저장하는 공간은 '데이터베이스' 라는 창고에 둠</li>
        <li>클라이언트-서버 아키텍처에 데이터베이스가 추가도니 형태를 3-Tier 아키텍처라고 함</li>
        <li>Java의 스프링 프레임워크</li>
        <li><span style={{ fontStyle: "italic" }}>참고: Dapp의 경우, 컨트랙트를 이용하여 데이터베이스 자체에서 데이터를 제공해줄 수 있기 때문에 "서버리스(Serverless) 아키텍처"라고 할 수 있다.</span></li>
      </ul>
      ...
      <h4>IP와 포트</h4>
      <ul><li>IP address(Internet Protocol address, IP 주소): 네트워크에 연결된 특정 PC의 주소를 나타내는 체계</li>
        <li>PORT: 그 주소에 진입할 수 있는 정해진 통로; 이미 사용 중인 포트는 중복해서 사용할 수 없음</li>
        <li>IPv4: 닷(.)으로 구분된 네 덩이의 숫자로 구분된 IP 주소 체계</li></ul>

      <h4>도메인과 DNS</h4>
      <ul>
        <li>IP 주소가 지번 또는 도로명 주소라면, 도메인 이름은 해당 주소에 위치한 상호</li>
        <li>nslookup: 터미널에서 도메인 이름을 통해 IP 주소를 확인하는 명령어
          <ul><li>nslookup google.com</li></ul>
        </li>
        <li>DNS: 대여한 도메인 이름과 매칭된 IP 주소를 확인하는 작업을 하는 서버</li>
      </ul>

      <h4>크롬 브라우저 에러 읽기</h4>
      <ul><li>chrome://network-errors/에서 에러 메시지를 검색할 수 있음</li></ul>

      <h4>SPA를 만드는 기술: AJAX</h4>
      <ul>
        <li>웹페이지 컴포넌트에서 필요한 데이터가 있으면 서버로부터 비동기적으로 가져올 수 있는 방법. "방법론"</li>
