...
const TIL0220 = () => {
    return (
        <div className='BlogDaily'>
            ...
            <ul><li>원시 자료형(primitive type) 예시:</li>
                <pre><code>{`
    // 원시 자료형(primitive type): number, string, boolean, undefined, null
    42, 'string', true, undefined, null
    `}</code></pre>
                <li>참조 자료형(reference type) 예시:</li></ul>
            <pre><code>{`
    // 참조 자료형(reference type)
    [0, 1, 2] // 배열
    {name: 'eliceKim', age: 45} // 객체
    function sum (x, y) { return x + y } // 함수
    `}</code></pre>

            <h4>스코프</h4>
            <ul><li>일반적으로 스코프는 사격 시 목표물을 정확히 조준하기 위해 사용</li>
                <li>자바스크립트에서의 스코프는 "변수의 유효범위"로 사용</li></ul>

            <p>변수 접근 규칙에 따른 유효 범위</p>
            <ul><li>바깥쪽 스코프에서 선언한 변수는 안쪽 스코프에서 사용 가능하지만</li>
                <li>안쪽 스코프에서 선언한 변수는 바깥쪽 스코프에서 사용 불가능함</li>
                <li>베그에서 안에 있음 밖이 보이지만 밖에선 안이 안 보이는 것과 비슷</li>
                <li>지역 변수는 전역 변수보다 더 높은 우선 순위를 가진다
                    <ul><li>지역 스코프에서 선언한 변수는 지역 변수</li>
                        <li>전역 스코프에서 선언한 변수는 전역 변수</li></ul>
                </li>
            </ul>

            <p>스코프의 종류</p>
            <ul><li>블록 스코프(Block Scope)
                <ul><li>중괄호로 둘러싼 밤위</li>
                    <li>들여쓰기 함</li></ul>
            </li>
                <li>함수 스코프(Function Scope)
                    <ul><li>function 키워드가 등장하는 함수 선언식 및 함수 표현식에서 형성하는 스코프</li>
                        <li>화살표 함수도 함수 스코프를 형성함</li></ul>
                </li>
            </ul>

            <p>const</p>
            <ul><li>constant 정의할 때 사용</li>
                <li>값의 재할당 불가능</li></ul>

            <h4>변수 선언 시 주의할 점</h4>
            <p>window 객체</p>
            <ul><li>브라우저에만 존재하는 객체</li>
                <li>개발자 도구를 열어서 window라고 입력하면 객체를 조회할 수 있다. </li>
                <li>함수 선언식으로 함수를 선언하거나 var로 선언된 전역 변수 및 전역 함수는 window 객체에 속하게 됨</li>
                <li>나중에 클라이언트 지갑, 메마 연결할 때 사용할 수 있음</li></ul>
